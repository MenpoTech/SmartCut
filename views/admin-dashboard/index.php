...
//                            'events'=>[['title'=> 'All Day Event', 'start'=>'2017-05-01']],
                            'ajaxEvents' => Url::to(['admin-dashboard/get-calendar'])
                        ]);
                        ?>
                    </div>
                </div>
...
<script>
    function popup_detail(tocr_no ,status) {
        ...
        $('.modal-header').html('<div class="pull-right"> <button aria-hidden="true" data-dismiss="modal" class="close" type="button" style="margin-top: 8px">X &nbsp;&nbsp;&nbsp;</button> </div> <h4 class="text-center bolder">TOCR : '+tocr_no+'</h4>');
        $.ajax({
            url:'<?= Yii::$app->urlManager->createUrl(['admin-dashboard/get-tocr-details']);?>',
            type: 'post',
    ...
    function load_assigned_test_details() {
        $.ajax({
            url:'<?= Yii::$app->urlManager->createUrl(['admin-dashboard/get-assigned-tocr-list']);?>',
            type: 'post',
            data: {type: 'Assigned'},
            beforeSend: function () {
    ...
    function load_received_test_details() {
        $.ajax({
            url:'<?= Yii::$app->urlManager->createUrl(['admin-dashboard/get-received-tocr-list']);?>',
            type: 'post',
            data: {type: 'Received'},
            beforeSend: function () {
    ...
    function load_witness_test_details() {
        $.ajax({
            url:'<?= Yii::$app->urlManager->createUrl(['admin-dashboard/get-witness-tocr-list']);?>',
            type: 'post',
            data: {type: 'Witness'},
            beforeSend: function () {
    ...
    function completed_test_details() {
        $.ajax({
            url:'<?= Yii::$app->urlManager->createUrl(['admin-dashboard/get-completed-tocr-list']);?>',
            type: 'post',
            data: {type: 'Completed'},
            beforeSend: function () {
