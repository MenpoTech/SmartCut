...
<div class="box-body">
    ...
    echo '<table class="kv-grid-table table table-bordered table-striped table-condensed kv-table-wrap table-hover ">
            ...
            <tr>
                ...
                <td>Sample Id</td>
                <td>Remarks</td>
                <td><label><input type="checkbox" onclick="check_all(this)"> &nbsp; Select All </label></td>
            </tr>
            </thead>
            <tbody>';
    $i=1;
    foreach($details as $dept_name=>$val) {
        ...
        foreach ($val as $id=>$value) {
            echo '<tr>
                ...
                <td>' . $value['remarks'] . '</td>
                <td><label>'.Html::checkbox('assigned_test_'.$id,false,['id'=>'test_assigned_'.$id,'value'=>$id,'class'=>'assigned_test checkbox1']).' Select </label></td>
              </tr>';
//            <td><span class="btn btn-block btn-primary btn-xs" onclick="receive_test(' . $id . ',\''.$value['tocr_number'].'\')"> Receive</span></td>
        }
    }
    echo '<tr><td colspan="4" align="right">'.Html::button('Close',['class'=>'btn btn-danger','data-dismiss'=>'modal']).'</td><td colspan="4" align="left">'.Html::button('Receive',['class'=>'btn btn-primary', 'onclick'=>'receive_multi_test(\''.$value['tocr_number'].'\')']).'</td></tr></tbody></table></div></div>';
