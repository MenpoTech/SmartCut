    ...
    echo '<div class="box-body">
        ...
        <table class="kv-grid-table table table-bordered table-striped table-condensed kv-table-wrap table-hover ">
            ...
            <tr>
                ...
                <td>Sample Id</td>
                <td>Remarks</td>
                <td><input type="checkbox" onclick="check_all(this)">Action</td>
            </tr>
            </thead><tbody>';
    $i=1;
...
//            <td><span class="btn btn-block btn-primary btn-xs" onclick="receive_test(' . $id . ',\''.$value['tocr_number'].'\')"> Receive</span></td>
    ...
    echo '<tr><td colspan="4" align="right">'.Html::button('Close',['class'=>'btn btn-danger','data-dismiss'=>'modal']).'</td><td colspan="4" align="left">'.Html::button('Receive',['class'=>'btn btn-primary', 'onclick'=>'receive_multi_test(\''.$value['tocr_number'].'\')']).'</td></tr></tbody> </table></div></div></div>';
}else {
